...
from evaluating_reasoning_models.evaluating_reasoning_models import extract_final_candidate, render_prompt
...
def self_refinement_loop(
    ...
    print("=" * 80)
    print("CURRENT FULL:")
    print(repr(current_full))
    print()
    print("CURRENT EXTRACTED:")
    print(repr(current_extracted))
    print()
    print("CURRENT SCORE:")
    ...
    for it in range(iterations):
        ...
        print("\nDRAFT FULL:")
        print(repr(draft_before_full))

        print("\nDRAFT EXTRACTED:")
        print(repr(draft_before_extracted))

        print("\nREVISED FULL:")
        print(repr(revised_full))

        print("\nREVISED EXTRACTED:")
        print(repr(revised_extracted))

        print("\nSCORES")
        print(f"score_before = {float(score_before):.4f}")
        print(f"score_after  = {float(revised_score):.4f}")

        # Recompute manually
        print("\nMANUAL RECOMPUTE")

        draft_full_manual = score_fn(
            prompt=prompt,
            answer=draft_before_full,
        )

        draft_extracted_manual = score_fn(
            prompt=prompt,
            answer=draft_before_extracted,
        )

        revised_full_manual = score_fn(
            prompt=prompt,
            answer=revised_full,
        )

        revised_extracted_manual = score_fn(
            prompt=prompt,
            answer=revised_extracted,
        )

        print(
            f"draft_full_score      = "
            f"{float(draft_full_manual):.4f}"
        )

        print(
            f"draft_extracted_score = "
            f"{float(draft_extracted_manual):.4f}"
        )

        print(
            f"revised_full_score    = "
            f"{float(revised_full_manual):.4f}"
        )

        print(
            f"revised_extracted_score = "
            f"{float(revised_extracted_manual):.4f}"
        )
        ...
        print("\nACCEPT?")
        print(
            f"{float(revised_score):.4f} >= "
            f"{float(current_score):.4f}"
        )
        print(accept)
